...
from lib.model.SCTformer import SCTformer
import matplotlib
...
def show3Dpose(vals, ax):
    ...
    ax.set_aspect('auto')  # msj oringin is equal
...
def get_pose3D(video_path, output_dir):
    sct = SCTformer(dim_in=3, dim_out=3, dim_feat=512, dim_rep=512,
                 ...
                 att_fuse=True, dct_ratio=0.6, freq_prune_layer=[0,1,2,3,4])

    if torch.cuda.is_available():
        sct = nn.DataParallel(sct)
    sct_chk_filename = "checkpoint/best_epoch.bin"
    checkpoint = torch.load(sct_chk_filename, map_location=lambda storage, loc: storage)
    sct.load_state_dict(checkpoint['model_pos'], strict=False)
    sct.cuda()
    sct.eval()
    ...
    for idx, clip in enumerate(clips):
        ...
        for j, post_out in enumerate(post_out_all):
            ...
            output_dir_3D = output_dir + 'pose3D_ours/'
    ...
    print('\nGenerating demo...')
    for i in tqdm(range(len(image_3d_gt_dir) - 1)):
        ...
        ## show
        font_size = 12
        fig = plt.figure(figsize=(12.8, 5.4))  # fig = plt.figure(figsize=(9.6, 5.4))
        ax = plt.subplot(121)
        ...
        plt.subplots_adjust(top=None, bottom=0.15, right=None, left=None, hspace=0, wspace=0)
        plt.margins(0, 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--video', type=str, default='dance.mp4', help='input video')
